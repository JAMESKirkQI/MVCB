...
class Learner(object):
    def __init__(self, unlabeled_dataloader, labeled_dataloader, projector, device, opt, cls=8):
        ...
        self.opt = opt
        self.cls = cls
        self.MVCB_loss = Loss(opt, device)
    ...
    def train(self):
        ...
        for k, (xu, target) in enumerate(self.unlabeled_dataloader):
            ...
            for i in range(self.opt.iteration // 30):
                ...
                if self.opt.GDC:
                    gdc_loss = self.MVCB_loss.gdc(ol, ou, self.opt.k_nums, self.opt.diffusion_iteration,
                                                   self.opt.alphaGDC,
                                                   self.opt.sigmaGD, self.opt.scale)
                    dic["GDC loss"] = gdc_loss.item()
                vscb_loss = self.MVCB_loss.vscb(ol, ou, label, target, self.label_propagation, k)
                dic["VSCB loss"] = vscb_loss.item()
                loss = vscb_loss + self.opt.lamb * gdc_loss
                dic["loss"] = loss.item()
                self.optimizer.zero_grad()
